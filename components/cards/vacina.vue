...
<template>
  <li class="relative flex space-x-6 py-6 xl:static">
    <div class="flex-auto">
      <h3 class="pr-10 font-semibold text-white xl:pr-0">{{ item.titulo }}</h3>
      <dl class="mt-2 flex flex-col text-gray-500 xl:flex-row">
        <div class="flex items-start space-x-3">
          ...
          <dd>
            <time :datetime="item.data">De {{ item.intervalo }} em {{ item.intervalo }} {{item.periodicidade}}</time>
          </dd>
        </div>
        <div
